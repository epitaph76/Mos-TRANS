...
## Диспетчерский BI-дашборд

В корне репозитория находится веб-дашборд на React, TypeScript и MapLibre. Он показывает воспроизведение исторического среза NDTP на карте Москвы, восстановленные GPS-траектории, положение ТС и карточку выбранного транспортного средства с расписанием остановок, телеметрией и демонстрационной оценкой отклонения. ML-модель пока не подключена.
...
Приложение использует готовые демонстрационные файлы в `src/data/`. Исходный архив датасета и локальные результаты обработки остаются вне Git.
